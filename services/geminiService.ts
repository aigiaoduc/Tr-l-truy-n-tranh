...
export const generateStoryPrompts = async (storyInput: string, style: IllustrationStyle, sceneCount: number, inputType: 'idea' | 'story'): Promise<StoryData> => {
  ...
  try {
    ...
    if (!parsedData.storyTitle || !parsedData.characterProfiles || !parsedData.scenes) {
        throw new Error("Cấu trúc dữ liệu nhận về từ API không hợp lệ.");
    }

    return parsedData as StoryData;
  } catch (error) {
    console.error("Lỗi khi tạo gợi ý câu chuyện:", error);
    throw new Error("Không thể tạo gợi ý câu chuyện. Model có thể không khả dụng hoặc yêu cầu đã bị chặn.");
  }
};